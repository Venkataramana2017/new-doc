...
Overview
This document describes on how to provision network and resources  commmon setup for a new account in AWS.

Follows AWS best practices such as:-

3-Tier Architecture
Appropriate Routing in Route Table etc
what are the resources needed(VPC/CIDR/Autoscalling/Security groups /S3/DynamoDB tables etc.).
IAM permitions 
Jenkins 
Terraform
BitBucket/GIT/GitHub( separe for reops for the resources line terrfrom Layer code and terrafrom Module code separate repos)


for provistionng using IAC - terrafrom 
...
If any shared service account using the follwoing stnderd procedure.
...
Create a BitBucket repo for Layer Code (tuinr-<account>-terraform-network (ex.terraform-network)), this is for specifying account specific variable and inputs for global network module
Shared-Services "SSH-Key" should be configured in BitBucket Repository Created above. This is required for Jenkins to clone the repo while running/executing the job
Repositories
...
Run Jenkins Job to provision VPC and its resources in Target Account.
  ...
  Updating Layer Code:
Copy the files for Layer code from any existing repo (Eg:uki-nordics-terraform-network)
...
subnet_names: Please follow  standard nome ,
